    ...
    for (mon in 1:12) {
      ...
      for (date in as.character(target_dates)) {
        ...
        for (ts in time_steps) {
          ...
          temp_nc <- tempfile(pattern = paste0("tmp_wind_", tm_str, "_"), fileext = ".nc") 
          ...
          # 파일 검증 (결측치 -9990 포함 여부와 관계없이 500바이트 초과 시 수집)
        ...
        if (length(daily_files) > 0) {
          ...
          uu_mean <- mean(r_stack[[seq(1, nlyr(r_stack), by = 4)]], na.rm = TRUE) / 10
          vv_mean <- mean(r_stack[[seq(2, nlyr(r_stack), by = 4)]], na.rm = TRUE) / 10
          
          ws_vector_mean <- sqrt(uu_mean^2 + vv_mean^2)
          wd_vector_raw <- atan2(uu_mean, vv_mean) * (180 / pi)
          wd_vector_mean <- (wd_vector_raw + 180) %% 360
          
          final_output <- c(uu_mean, vv_mean, wd_vector_mean, ws_vector_mean)
          names(final_output) <- c("uu", "vv", "wd", "ws")
          
          writeCDF(final_output, daily_save_path, 
                   overwrite = TRUE, 
                   varname = "wind",
                   unit = c("m/s", "m/s", "deg", "m/s"),
                   compression = 9)
        ...
        # 메모리 효율화를 위한 객체 제거 및 가비지 컬렉션
        rm(daily_files, r_stack, uu_mean, vv_mean, ws_vector_mean, wd_vector_mean, final_output)
        gc()
      }
    }
  }
