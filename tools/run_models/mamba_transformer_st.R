...
load_and_prepare_data <- function(cfg = default_st_config) {
	...
	dt <- as.data.table(arrow::read_parquet(
		file.path(cfg$basepath, cfg$filename), as_data_frame = TRUE
	))
...
spatiotemporal_dataset <- dataset(
	...
	initialize = function(dt, nn_info, feat_sets, seq_len, cfg) {
		...
		cat(sprintf("Dataset: %d valid samples from %d stations\n",
	...
	.getitem = function(i) {
		...
		n_nb_feats <- ncol(self$station_data[[as.character(
			self$nn_info$nn_idx[s$station_idx, 1])]]$nb_mat)
...
run_st_experiment <- function(cfg = default_st_config,
	...
	if (length(good_stations) < cfg$K_neighbors + 1L) {
		stop(sprintf("Need at least %d stations, got %d",
	...
	# --- Rolling mean ---
	all_covars <- union(feat_sets$static_covars,
	...
	cat(sprintf("Feature dims: target_temporal=%d, neighbor_temporal=%d, static=%d\n",
	...
	for (tgt in names(metrics)) {
		m <- metrics[[tgt]]
		cat(sprintf("%s | RMSE: %.4f | MAE: %.4f | R2: %.4f | n=%d\n",
